...
import { CreateClinicDTO } from "@/services/clinicService/dto";
import { Col, DatePicker, Form, FormProps, Input, Row } from "antd";

interface Props extends FormProps<CreateClinicDTO> {
  requiredPassword?: boolean;
}

export const ClinicForm = ({ requiredPassword = true, ...rest }: Props) => {
  return (
    <Form layout="vertical" {...rest}>
      <Row gutter={[16, 16]}>
        <Col span={24} md={{ span: 12 }}>
          <Form.Item
            label="Nome da Clinicm"
            name={"name"}
            key={"name"}
            id="name"
            rules={[{ required: true, message: "Campo obrigatório!" }]}
          >
            <Input placeholder="Nome da Clinicm" />
          </Form.Item>
        </Col>
      </Row>
